routerAdd(
  ...
  (e) => {
    ...
    var correlationKey = 'TESTE-2D2A-R12-' + ts + '-' + nonce
    ...
    function checkLock() {
      try {
        var rec = $app.findFirstRecordByData('com_parametros', 'chave', 'ac_r12_execution_lock')
    ...
    function engageLock() {
      try {
        ...
        var fr
        try {
          fr = $app.findFirstRecordByData('com_parametros', 'chave', 'ac_r12_execution_lock')
        } catch (_) {
          fr = new Record(pc)
          fr.set('chave', 'ac_r12_execution_lock')
          fr.set('versao', 1)
        }
        fr.set('valor', 'locked')
        fr.set('ativo', true)
        fr.set('descricao', 'Server-side single-execution lock for R12 runner')
        fr.set('tipo', 'lock')
        $app.save(fr)
        return true
      } catch (_) {
    ...
    if (checkLock()) {
      return e.json(200, {
        runner_version: 'R12',
        route: '/backend/v1/integracao/ac/run-round-2d2a-r12',
        execution_enabled: false,
        executed: true,
        locked: true,
        message: 'R12 already executed — server-side single-execution lock prevents re-execution',
    ...
    if (!EXECUTION_ENABLED) {
      return e.json(200, {
        ...
        activecampaign_calls: 0,
        message:
          'R12 runner ready — execution disabled. Set EXECUTION_ENABLED=true and release lock to run.',
      })
    }
      ...
      if (overallStatus === 'PASS') {
        var t7Payload = {
          ...
          contact: {
            ...
            firstName: '[TESTE]',
            lastName: 'R12MissingSig',
            email: 'teste-r12-sm@teste.local',
            phone: '+5511999999999',
          },
        }
        ...
        t7.payload_is_full_semantic_copy =
          'Exact copy of positive control payload (contact_create) with IDs swapped to R12 correlation key'
        t7.timestamp_note =
      ...
      if (overallStatus === 'PASS') {
        var t8Payload = {
          ...
          contact: {
            ...
            firstName: '[TESTE]',
            lastName: 'R12InvalidSig',
            email: 'teste-r12-is@teste.local',
            phone: '+5511999999999',
          },
        }
        ...
        t8.payload_is_full_semantic_copy =
          'Exact copy of positive control payload (contact_create) with IDs swapped to R12 correlation key; signature header present but tampered'
      ...
      if (overallStatus === 'PASS' && mode === 'full') {
        ...
        if (overallStatus === 'PASS') {
          var up = sign({
            ...
            deal: {
              id: dealExtId,
              title: '[TESTE] R12 Full Atualizado',
              value: 15000,
        ...
        if (overallStatus === 'PASS') {
          ...
          if (dV3) {
            try {
              ...
              dealRestored =
                negRec.getString('titulo') === '[TESTE] R12 Full Negocio' &&
