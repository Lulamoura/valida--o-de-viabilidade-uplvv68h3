routerAdd(
  'POST',
  '/backend/v1/integracao/ac/run-round-2d2a-r6',
  (e) => {
    ...
    var correlationKey = 'TESTE-2D2A-R6-' + ts + '-' + nonce
    ...
    function buildTest(
      ...
      return {
        ...
        body_sanitized: bodyStr ? bodyStr.substring(0, 200) : '',
    ...
    try {
      ...
      if (overallStatus === 'PASS') {
        var t7Body = JSON.stringify({
          type: 't',
          contact: { id: correlationKey + '-SM-001', firstName: '[TESTE]', lastName: 'MissingSig' },
          timestamp: new Date().toISOString(),
        })
        var t7s = new Date().toISOString(),
          fb7 = readFlag(),
          cb7 = getCounts()
        var r7 = callWH('POST', { 'Content-Type': 'application/json' }, t7Body)
        var ca7 = getCounts(),
          fa7 = readFlag(),
        ...
        t7.content_length_note =
          'Content-Length computed automatically by $http.send runtime; not explicitly set by caller'
        t7.signature_header_present = false
        t7.timestamp_note =
          'Timestamp present in payload but not evaluated — signature absence is validated before timestamp check'
      ...
      if (overallStatus === 'PASS' && mode === 'full') {
        ...
        if (overallStatus === 'PASS') {
          var up = sign({
            ...
            deal: {
              id: dealExtId,
              title: '[TESTE] R6 Full Atualizado',
              value: 15000,
        ...
        if (overallStatus === 'PASS') {
          ...
          if (dV3) {
            try {
              ...
              dealRestored =
                negRec.getString('titulo') === '[TESTE] R6 Full Negocio' &&
