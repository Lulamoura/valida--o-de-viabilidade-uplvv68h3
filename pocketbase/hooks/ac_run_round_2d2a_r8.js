routerAdd(
  'POST',
  '/backend/v1/integracao/ac/run-round-2d2a-r8',
  (e) => {
    ...
    if (!EXECUTION_ENABLED) {
      return e.json(200, {
        build_status: 'READY',
        runner: 'R8',
        route: '/backend/v1/integracao/ac/run-round-2d2a-r8',
        executed: false,
      })
    }
    ...
    var correlationKey = 'TESTE-2D2A-R8-' + ts + '-' + nonce
    ...
    var POSITIVE_CONTROL_PAYLOAD = {
      type: 'contact_create',
      contact: {
        id: correlationKey + '-PC-001',
        firstName: '[TESTE]',
        lastName: 'R8PositiveControl',
        email: 'teste-r8-pc@teste.local',
        phone: '+5511999999999',
      },
    }
      ...
      if (overallStatus === 'PASS') {
        var t7Payload = {
          ...
          contact: {
            ...
            firstName: '[TESTE]',
            lastName: 'R8MissingSig',
            email: 'teste-r8-sm@teste.local',
            phone: '+5511999999999',
          },
        }
        ...
        var t7Headers = {
          'Content-Type': 'application/json',
          'Content-Length': String(t7ByteLen),
        }
        ...
        t7.explicit_content_length = t7ByteLen
        t7.content_length_note =
          'Content-Length explicitly set to ' +
          t7ByteLen +
          ' bytes (ASCII JSON body); X-AC-Signature header omitted'
        t7.signature_header_present = false
        t7.payload_is_full_semantic_copy =
          'Exact copy of positive control payload (contact_create) with IDs swapped to R8 correlation key'
        t7.timestamp_note =
        ...
        if (t7EventoCreated || t7VinculoCreated) {
          ...
          overallStatus = 'FAIL'
          stopReason = 'Test 7 FAIL: expected 401 got ' + r7.status
        }
      }
      ...
      if (overallStatus === 'PASS' && mode === 'full') {
        ...
        if (overallStatus === 'PASS') {
          var up = sign({
            ...
            deal: {
              id: dealExtId,
              title: '[TESTE] R8 Full Atualizado',
              value: 15000,
        ...
        if (overallStatus === 'PASS') {
          ...
          if (dV3) {
            try {
              ...
              dealRestored =
                negRec.getString('titulo') === '[TESTE] R8 Full Negocio' &&
