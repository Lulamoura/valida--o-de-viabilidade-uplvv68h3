...
function propostaPodeAcessar(user, perfil, negocio) {
  ...
  return (
    !!user.getString('equipe_id') && negocio.getString('equipe_id') === user.getString('equipe_id')
  )
}
...
routerAdd(
  ...
  (e) => {
    ...
    if (known.length) {
      ...
      if (known[0].getString('estado') !== 'concluido') return e.json(409, { error: 'CONCORRENTE' })
