...
const SESSION_KEY = 'comercial_2d2a_r10_http_response_raw'
const ROUTE_PATH = '/backend/v1/integracao/ac/run-round-2d2a-r10'

function loadFromSession(): R10Evidence | null {
  try {
    const raw = sessionStorage.getItem(SESSION_KEY)
    if (!raw) return null
    const parsed = JSON.parse(raw) as R10Evidence
...
function saveToSession(data: R10Evidence) {
  try {
...
export function R10EvidenceBlock() {
  const [evidence, setEvidence] = useState<R10Evidence | null>(null)
  ...
  const handleExecute = async () => {
    ...
    try {
      ...
      if (res.ok) {
        toast.success('R10 Round executado — resposta HTTP capturada')
      } else {
        toast.warning(`R10 Round retornou HTTP ${res.status} — resposta capturada`)
      }
    } catch (err) {
      ...
      saveToSession(data)
      setEvidence(data)
      toast.error('Erro de rede ao executar R10 round')
    } finally {
      setExecuting(false)
    }
  }
  ...
  const handleDownload = () => {
    ...
    a.href = url
    a.download = `r10-http-response-${evidence.captured_at.replace(/[:.]/g, '-')}.json`
    ...
    <Card>
      <CardHeader>
        <CardTitle className="flex items-center gap-2">
          ...
          Full Round (R10) — Resposta HTTP Capturada
        </CardTitle>
      </CardHeader>
      <CardContent className="space-y-3">
        <div className="flex flex-wrap items-center gap-2">
          <AlertDialog open={dialogOpen} onOpenChange={setDialogOpen}>
            ...
            <AlertDialogContent>
              <AlertDialogHeader>
                <AlertDialogTitle>Confirmar execução R10</AlertDialogTitle>
                <AlertDialogDescription>
                  Esta ação executará o round R10 completo (security matrix + fluxo funcional) com
        ...
        {!evidence && !executing && (
          <div className="text-sm text-muted-foreground">
            Nenhuma evidência R10 capturada. Clique em "Full Round (R10)" e confirme para executar.
          </div>
        )}
      </CardContent>
    </Card>
