...
function loadFromSession(): R11Evidence | null {
  try {
    ...
    if (!raw) return null
    const parsed = JSON.parse(raw) as R11Evidence
...
function saveToSession(data: R11Evidence) {
  try {
...
export function R11EvidenceBlock() {
  const [evidence, setEvidence] = useState<R11Evidence | null>(null)
  ...
  const handleExecute = async () => {
    ...
    try {
      ...
      if (res.ok) {
        toast.success('R11 Round executado — resposta HTTP capturada')
      } else {
        toast.warning(`R11 Round retornou HTTP ${res.status} — resposta capturada`)
      }
    } catch (err) {
      ...
      saveToSession(data)
      setEvidence(data)
      toast.error('Erro de rede ao executar R11 round')
    } finally {
      setExecuting(false)
    }
  }
  ...
  const handleDownload = () => {
    ...
    a.href = url
    a.download = `r11-http-response-${evidence.captured_at.replace(/[:.]/g, '-')}.json`
      ...
      <CardContent className="space-y-3">
        <div className="flex flex-wrap items-center gap-2">
          <AlertDialog open={dialogOpen} onOpenChange={setDialogOpen}>
            ...
            <AlertDialogContent>
              <AlertDialogHeader>
                <AlertDialogTitle>Confirmar execução R11</AlertDialogTitle>
                <AlertDialogDescription>
                  Esta ação executará o round R11 completo (security matrix + fluxo funcional) com
